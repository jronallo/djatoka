...
== IIIF Support
This gem can now translate parameters from {International Image Interoperability Framework (IIIF)}[http://www-sul.stanford.edu/iiif/image-api/]  requests into Djataka requests.  It follows the same pattern as a regular Djatoka::Region
...
== IIIF Example
  ...
  # If you want the xml flavor of a IIIF Info request
  # Use Djatoka::Metadata#to_iiif_xml.  It can be called with the same type of block as #to_iiif_json

  # IIIF Image Request
  # Create a IiifRequest with the resolver and id
  iiif_request = Djatoka::IiifRequest.new(resolver, identifier)

  # Set IIIF paramaters and create a Djatoka::Region (Note: All IIIF parameters are required before creating a Djatoka::Region )
